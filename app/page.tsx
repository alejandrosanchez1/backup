...
export default function GymProApp() {
  ...
  // Añade esto cerca de tus otros useEffect
useEffect(() => {
  if (currentView === 'exercises') {
    fetchExercises(); // Carga todos los ejercicios al entrar a la vista
  }
}, [currentView]);
  ...
  if (isActive) {
    // Solo inicia el intervalo si isActive es TRUE
    interval = setInterval(() => {
      setWorkoutTimer((prev) => prev + 1);
    }, 1000);
  } else {
    // Si isActive es FALSE, limpiamos el intervalo
...
const toggleLanguage = () => {
  ...
  setWorkoutData((prev: any) => {
    ...
    // Si marcamos como completado y no lo estaba antes
    if (newData.completed === true && !currentSets[setIndex].completed) {
      
      // BUSCAMOS EL TIEMPO DE DESCANSO CONFIGURADO
      const exerciseConfig = activeRoutine.exercises.find(
        (e: any) => e.id === exerciseId || e.exercise_id === exerciseId
      );
      
      // Usamos el tiempo de la rutina o 60 por defecto si no hay nada
...
const fetchHistory = async () => {
  if (!user) return;
  const { data, error } = await supabase.from('workout_logs').select('*').eq('user_id', user.id).order('date', { ascending: false });
  if (error) {
...
const finishWorkout = async () => {
  ...
  try {
    ...
    confetti({
      ...
      spread: 70,
      origin: { y: 0.6 },
      colors: ['#10b981', '#3b82f6', '#ffffff'] // Verde esmeralda, azul y blanco
    });
...
const streak = useMemo(() => {
  ...
  try {
    const dates = Array.from(new Set(history.map(l => l.date.split('T')[0]))).sort().reverse();
  ...
  return (
    <div className="h-screen bg-gray-900 flex items-center justify-center p-6 text-center">
      <div className="bg-gray-800 p-8 rounded-3xl border border-gray-700 w-full max-w-md space-y-6">
        ...
        <button onClick={handleLogin} className="w-full bg-emerald-500 py-4 rounded-xl font-black uppercase text-white hover:bg-emerald-600">Entrar</button>
      </div>
    </div>
  );
}
      ...
      <div className="bg-gray-800 p-6 rounded-3xl border border-gray-700 space-y-4 shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gray-900 p-4 rounded-xl border border-gray-700 col-span-2">
            <p className="text-[10px] text-gray-500 uppercase font-bold">Índice de Masa Corporal (IMC)</p>
...
{currentView === 'routines' && (
  <div className="space-y-6 animate-in fade-in">
    <div className="flex justify-between items-center">
      <h1 className="text-3xl font-bold uppercase tracking-tighter">{t('settings')}</h1>
      <button onClick={saveProfile} className="bg-emerald-500 px-6 py-2 rounded-xl font-bold flex items-center gap-2 active:scale-95 transition-all">
        <Save size={18} /> {t('save')}
      </button>
    </div>
  ...
  <div className="space-y-6">
    <div className="grid grid-cols-1 gap-4">
      <div>
        ...
        <input 
          className="w-full bg-gray-900 border border-gray-700 rounded-xl p-3 outline-none focus:border-emerald-500" 
    ...
    <div className="grid grid-cols-2 gap-4">
      <div>
        ...
        <input 
          type="number" 
          step="0.1"
          className="w-full bg-gray-900 border border-gray-700 rounded-xl p-3 outline-none focus:border-emerald-500 text-emerald-400 font-bold" 
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      {/* --- TAB OBJETIVOS (La que te salía vacía) --- */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-3">
              ...
              {Object.keys(userStats.skinfolds).map(k => (
                <div key={`skinfold-${k}`} className="flex justify-between items-center"> {/* AÑADIDA KEY ÚNICA */}
                  <span className="text-[10px] text-gray-400 capitalize">{k}</span>
                  <input 
                    type="number" 
                    className="w-16 bg-gray-900 border border-gray-700 rounded-lg p-1 text-center text-xs font-bold" 
                    value={(userStats.skinfolds as any)[k]} 
                    onChange={e => setUserStats({...userStats, skinfolds: {...userStats.skinfolds, [k]: e.target.value}})} 
                  />
                </div>
              ))}
            </div>
            <div className="space-y-3">
              ...
              {Object.keys(userStats.perimeters).map(k => (
                <div key={`perimeter-${k}`} className="flex justify-between items-center"> {/* AÑADIDA KEY ÚNICA */}
                  <span className="text-[10px] text-gray-400 capitalize">{k}</span>
                  <input 
                    type="number" 
                    className="w-16 bg-gray-900 border border-gray-700 rounded-lg p-1 text-center text-xs font-bold" 
                    value={(userStats.perimeters as any)[k]} 
                    onChange={e => setUserStats({...userStats, perimeters: {...userStats.perimeters, [k]: e.target.value}})} 
                  />
                </div>
              ))}
            </div>
      ...
      {activeSettingsTab === 'rutinas' && (
          ...
          {routines.map(r => (
            <div key={r.id} className="bg-gray-900/50 p-4 rounded-2xl border border-gray-700 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={() => { setShowEditRoutineModal(r.id); setCurrentView('exercises'); }} className="p-2 bg-blue-500/10 text-blue-400 rounded-lg text-[10px] font-black uppercase">+ Añadir</button>
                <button onClick={() => setShowEditRoutineModal(r.id)} className="p-2 bg-gray-700 text-white rounded-lg"><Settings size={18}/></button>
              </div>
            </div>
          ))}
        </div>
        ...
        {currentView === 'exercises' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="flex justify-between items-center"><h1 className="text-3xl font-black uppercase tracking-tighter">{t('library')}</h1><button onClick={() => setShowCreateExModal(true)} className="bg-emerald-500 p-3 rounded-2xl text-white shadow-lg flex items-center gap-2 font-bold text-xs uppercase"><Plus size={18}/> {t('createEx')}</button></div>
            <div className="relative"><Search className="absolute left-3 top-3 text-gray-500" /><input placeholder={t('search')} className="w-full bg-gray-800 border border-gray-700 rounded-xl py-3 pl-10 pr-4 outline-none font-bold" onChange={e => fetchExercises(e.target.value)} /></div>
            <div className="space-y-3">
              {results.map(ex => (
                <div key={ex.id} className="flex items-center gap-4 bg-gray-800 p-3 rounded-2xl border border-gray-700 group">
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-gray-900 flex-shrink-0">{ex.gifUrl && <img src={ex.gifUrl} className="w-full h-full object-cover" alt="" />}</div>
                  ...
                  {showEditRoutineModal && <button onClick={async () => { await supabase.from('routine_exercises').insert([{ routine_id: showEditRoutineModal, exercise_id: ex.id, sets: 3, reps: '12', rest_time: 60 }]); fetchRoutines(user.id); Toast.show({text: 'Añadido'}); }} className="bg-emerald-500 p-3 rounded-xl text-white shadow-lg"><Plus size={20}/></button>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-2xl mx-auto w-full space-y-8 animate-in slide-in-from-bottom-10 pb-32">
            ...
            <div className="sticky top-0 z-30 bg-gray-900/95 backdrop-blur-xl border-b border-gray-800/50 px-6 py-6 rounded-b-[2.5rem] shadow-2xl">
              <div className="flex justify-between items-center mb-4">
                ...
                {/* Botón Finalizar (Solo si hay progreso) */}
                <button 
              ...
              {/* CONTROLADOR DE TIEMPO (Play/Pause) */}
              ...
              {activeRoutine.exercises.map((exercise: any) => {
                ...
                return (
                  <div key={`workout-ex-${exercise.id}`} className="bg-gray-800/40 rounded-[2.5rem] border border-gray-700/50 overflow-hidden shadow-xl backdrop-blur-sm">
                    ...
                    {/* CABECERA DEL EJERCICIO (Imagen + Nombre + Tags) */}
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-[999] h-20 bg-[#111827] border-t border-gray-800 flex items-center justify-around px-2 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
      ...
      <button type="button" onClick={() => setCurrentView('routines')} className="flex flex-col items-center justify-center w-full h-full gap-1"><Settings className={`w-6 h-6 ${currentView === 'routines' ? 'text-emerald-500' : 'text-gray-500'}`} /><span className="text-[10px] font-bold uppercase">{t('settings')}</span></button>
      <button type="button" onClick={() => setCurrentView('exercises')} className="flex flex-col items-center justify-center w-full h-full gap-1"><Library className={`w-6 h-6 ${currentView === 'exercises' ? 'text-emerald-500' : 'text-gray-500'}`} /><span className="text-[10px] font-bold uppercase">{t('library')}</span></button>
      <button type="button" onClick={() => setCurrentView('progress')} className="flex flex-col items-center justify-center w-full h-full gap-1"><User className={`w-6 h-6 ${currentView === 'progress' ? 'text-emerald-500' : 'text-gray-500'}`} /><span className="text-[10px] font-bold uppercase">{t('profile')}</span></button>
    </nav>
      ...
      <div className="fixed inset-0 bg-black/90 z-[2000] flex items-center justify-center p-4 backdrop-blur-md">
        <div className="bg-gray-800 w-full max-w-md rounded-3xl border border-gray-700 p-6 space-y-6 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <select title="target" className="w-full bg-gray-900 border border-gray-700 rounded-xl p-3 outline-none" value={newExData.target} onChange={e => setNewExData({...newExData, target: e.target.value})}>{MUSCLE_OPTIONS.map(m => <option key={m} value={m}>{m}</option>)}</select>
    ...
    {isResting && (
      <div className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-orange-600 text-white px-8 py-4 rounded-full shadow-2xl flex items-center gap-6 z-[1100] animate-bounce border-2 border-white/20">
        <div className="flex items-center gap-3"><Timer className="w-6 h-6" /><span className="text-3xl font-mono font-black">{restTimer}s {/* <--- Que use el estado dinámico */} </span></div>
