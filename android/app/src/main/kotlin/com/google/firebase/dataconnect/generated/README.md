...
with service ID `app-similar-to-hevy`
hosted in `us-east4`.
...
is the query named "GetMyGoals",
which can be accessed via the [ExampleConnector.getMyGoals] property.


An example of the property for a mutation
is the mutation named "CreateNewWorkout",
which can be accessed via the [ExampleConnector.createNewWorkout] property.
...
For example, the "GetMyGoals" query has no variables
and can be executed via the
[ExampleConnector.getMyGoals]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getMyGoals.execute()
println("GetMyGoals query returned: ${queryResult.data}")
```
...
For example, the "CreateNewWorkout" mutation has 3 required variables ("workoutDate", "workoutType", and "durationMinutes")
and can be executed via the [ExampleConnector.createNewWorkout]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createNewWorkout.execute(workoutDate=LocalDate(2003, 5, 12), workoutType="quux", durationMinutes=9956)
println("CreateNewWorkout mutation returned: ${mutationResult.data}")
```
...
For example, the "RecordWorkoutExercise" mutation has 6 optional variables ("sets", "reps", "weightKg", "distanceKm", "durationMinutes", and "notes")
and can be executed via the [ExampleConnector.recordWorkoutExercise]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.recordWorkoutExercise.execute(workoutId=java.util.UUID.randomUUID(), exerciseId=java.util.UUID.randomUUID()) {
  sets = 4893
  reps = 2882
  weightKg = 9923.97
  distanceKm = 90.54
  durationMinutes = 9956
  notes = "foo"
}
println("RecordWorkoutExercise mutation returned: ${mutationResult.data}")
```
